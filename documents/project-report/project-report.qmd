...
| --- | -------------- | ----------------------------------------------------- |
| 1   | M        | The system **must** be able to count the amount of people in the crowd |
|     |          |                                                             |
| 2   | M        | The system **must** be able to segment the crowd into virtual sections of equal physical size |
|     |          |                                                             |
| 3   | M        | The system **must** be able to create a heatmap of the crowd density |
|     |          |                                                             |
| 4   | S        | The system **should** be able to detect the movement of the virtual crowd sections |
|     |          |                                                             |
| 5   | S        | The system **should** be able to distinguish different types of movement |
|     |          |                                                             |
| 6   | S        | The system **should** be able to detect choke points in the crowd movements |
|     |          |                                                             |
| 7   | S        | The system **should** be able to generate a summarizing report of the concert with statistics of (crowd density, crowd count, risk factors, choke points, and other) relevant safety indicators after the event |
|     |          |                                                             |
| 8   | C        | The system **could** be able to estimate a numerical risk factor based on available factors |
|     |          |                                                             |
| 9   | C        | The system **could** be able to generate a live, or delayed, video overlaid user interface |
|     |          |                                                             |
| 10  | W        | The system **will not** be able to identify dangerous situations that might call for cautionary actions such as crowd surge, mosh pits, falls, blocked exits, etc. |
|     |          |                                                             |
| 11  | W        | The system **will not** be able to use data gathered elsewhere at a concert such as alcohol sales, average crowd age, sound, artists, etc. to give a more precise crowd profile and thus risk factor |
...
| **ID**   | **MoSCoW**   | **Requirement**  |
| --- | -------------- | ----------------------------------------------------- |
| 1   | M        | The system **must** protect the privacy of the CCTV video feed |
|     |          |                                                             |
...
|     |          |                                                             |
| 5   | C        | The system **could** have high reliability that is not based on the visual circumstances and environment (e.g. sunlight, stage light, audience flashlights, and other visual effects) |
|     |          |                                                             |
...
### Crowd counting
**TODO:** Add defintion of crowd counting
...
In [@li2021approaches] the current leading approach to crowd counting is the use of a CNN. This paper, contrary to our definiton of crowd counting, includes object detection as a method of crowd counting. It also includes regression based methods and of course CNN's. The main challenge with CNN's for crowd counting is still to differentiate between the large scale variations in the countable human heads. [@li2021approaches] discusses several categories of approaches for CNN based models to solve the scale variation problems including multi-scale fusion. This approach is the main category that the paper recommends for future research, as it shows promising results in the balance between performance and accuracy.

Multi-scale fusion for SASNet.

Use Crowdcounting and NOT objectcounting models. Object counting is better for segmenting visually clear definitions and not individual members of a crowd
