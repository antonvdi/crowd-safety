...
## Choosing a technology for Crowd Counting
...
**Block Size**
Might be wrong or uneeded

When working with deep learning, block size is an important factor. Most deep learning model are unable to handle sequential input of variable length, this is where block size is relevant. Block size makes sure that the input is always the same regardless of original size. See example:

The following input is not the same size

```python
hello world
This is a cool project
```
With a block size of 10, the input would instead look like this: 
...
As such, a block size makes it significantly easier for the deep learning model to handle the input. 
