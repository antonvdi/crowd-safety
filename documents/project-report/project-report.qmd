...
**Nr1 The system must be able to count the amount of people in the crowd with a precision required by the user base**\

To count the people in a crowd, the SASNet crowd counting model mentioned in @sec-model-selection  was implemented. This is done in the model_wrapper.py class. The model_wrapper class has a get_model class that returns the model for use in the camera class.
Before we can use the model on our data, we have to preprocess it. As the SASNet model is made for singular images, we needed to convert our videos to a proper format, this is done using torch.utils.dataset library. This iterable dataset allows for loading a frame interval from a video into an array of images using cv2. Using cv2, we are also able to resize the video to a specified resolution for easier handling later on.\
Once we have a specified frame interval from the video ready for processing, we continue to the camera class. \

In the camera class we use both the model, dataset from before and the CameraUtils class.



**Nr 4 The system should be able to calculate the physical density of humans pr. area unit**\

### Downscaling & Pespective Correction
...
## Accuracy of Crowd Count
TODO: Count a small section and compare with model output. Compare differences during high and low density. Do it under good circumstances (good lighting, few compression artifacts and low noise, low amount of "stage noise"). Note limitations on bad circumstances. 
...
It is also important to note that the required uncertainty percentages varies depending on the total size of the crowd. A 
