...
When starting this project, we originally focused on the usage of Metas SAM model to be the main driver. During our first research sessions, we discovered that SAM was not adequate in counting people but better suited for object detection. the following citation from an article we read highlights the problems well: 
...
The first implementation was setup in Google [@Colab]. Colab provides free access to CUDA powered GPU power, which is needed for the implementation as it relies on [tensorflow](https://www.tensorflow.org) and [pyTorch](https://pytorch.org). specifically, pyTorch needs CUDA to function. Our implementation is based on an opensource implementation of [@sasnet] found [here](https://github.com/TencentYoutuResearch/CrowdCounting-SASNet). 
...
#| 
  with torch.no_grad():
    ...
    for i_img in range(pred_map.shape[0]):
        ...
        original_image = cv2.imread(img_path)  # Replace 'your_original_image.jpg' with your image file path
        ...
        cv2.imwrite('overlayed_image3.jpg', blended_image)
...
![Original drone image](../images/green1.png){height=50% #green1}

**SASNet image**

![Overlayed image](../images/overlayed_image2.jpg){height=50% #Overleayed_image}
